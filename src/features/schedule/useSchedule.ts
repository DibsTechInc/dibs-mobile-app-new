...
/**
 * ONE definition of the schedule query, shared by the hook and the launch warm-up
 * (`AppWarmup`). Two copies of the key or the staleTime is how a prefetch ends up warming a
 * cache entry no screen ever reads.
 */
export const scheduleQueryOptions = (timeZone: string) =>
  queryOptions({
...
export function useSchedule() {
  ...
  return useQuery(scheduleQueryOptions(timeZone));
}
